...
namespace HLSH_hashing {
    ...
    struct SegmentAllocator {
        ...
        void PreAllocateSegment(size_t thread_id) {
            ...
            // s2: preallocate memory for each list if used number exceed the
            // threshold
            while (1) {
              for (size_t i = 0; i < kThreadNum; i++) {
        ...
        SegmentAllocator<KEY, VALUE>() {
            ...
            seg_thread = std::thread(
                &SegmentAllocator<KEY, VALUE>::PreAllocateSegment, this, 35);
#endif
        }
    ...
    struct DirectoryAllocator {
        ...
        DirectoryAllocator(size_t _depth) {
            ...
            for (size_t i = 0; i < kDirArraySize; i++) {
                ...
                if (ret) {
                    printf(
                        "allocation directory failure, depth: %lu, code: %d\n",
                        depth, ret);
                }
                memset(d[i], 0,
                       sizeof(Directory<KEY, VALUE>) +
        ...
        inline Directory<KEY,VALUE>* Get() {
            ...
            while (b) { b = is_empty[index].load(std::memory_order_acquire); } //wait new directory auto s = d[index];
        ...
        void DirAlloc(size_t thread_id) {
            ...
            while (1) {
                ...
                if (nd) {
                  ...
                  t->lock.GetLock();
                  size_t chunk_size =
                      pow(2, nd->global_depth - t->si.local_depth);
                  // if (nd->_[i]) { t->lock.ReleaseLock(); i = i + chunk_size;
                  // continue; }
                  size_t start_pos = t->si.pattern
                                     << (nd->global_depth - t->si.local_depth);
